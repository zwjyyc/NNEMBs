...
def create_batches(perm, x, y, batch_size, isTrain = True):
...
class Model:
    ...
    def eval_macro_recall(self, preds, golds):
	fine_0 = sum([p == y if y==0 else 0 for ps, ys in zip(preds, golds) for p, y in zip(ps, ys)]) + 0.0
	fine_tot_0 = sum([1 if y==0 else 0 for ps, ys in zip(preds, golds) for p, y in zip(ps, ys)]) 
        fine_1 = sum([p == y if y==1 else 0 for ps, ys in zip(preds, golds) for p, y in zip(ps, ys)]) + 0.0
        fine_tot_1 = sum([1 if y==1 else 0 for ps, ys in zip(preds, golds) for p, y in zip(ps, ys)])
        fine_2 = sum([p == y if y==2 else 0 for ps, ys in zip(preds, golds) for p, y in zip(ps, ys)]) + 0.0
        fine_tot_2 = sum([1 if y==2 else 0 for ps, ys in zip(preds, golds) for p, y in zip(ps, ys)])
	return fine_tot_0, fine_0 / fine_tot_0, fine_tot_1, fine_1 / fine_tot_1, fine_tot_2, fine_2 / fine_tot_2

    def result2file(self, preds, predps, golds, filename):
	with open(filename, 'w') as fout:
	    for ps, pps, ys in zip(preds, predps, golds):
		for p, y, pp in zip(ps, ys, pps):
		    fout.write(str(p) + '\t' + str(y) + '\t')
		    for ppp in pp:
			fout.write(str(100.0 * round(ppp, 5)) + ' ')
		    fout.write('\n')
    ...
    def train(self, train, dev, test):
        ...
        eval_acc = theano.function(
             inputs = [self.x],
             outputs = [self.pred, self.p_y_given_x],
             allow_input_downcast = True
        )
	...
	if args.load:
	    ...
	    predss = [ eval_acc(x) for x in dev_batches_x ]
	    preds = [item[0] for item in predss]
	    predps = [item[1] for item in predss]
            self.result2file(preds, predps, dev_batches_y, args.result + '.dev')
	    nowf_dev = self.eval_accuracy(preds, dev_batches_y)
            preds = self.eval_macro_recall(preds, dev_batches_y)
            pred = (preds[1] + preds[3] + preds[5]) / 3
            best_dev = pred
            say("\tdev accuracy/negativeN/negativeR/neutralN/neturalR/positiveN/positiveR/macroR/bestMarcroR=%.4f\t%d\t%.4f\t%d\t%.4f\t%d\t%.4f\t%.4f\tbest=%.4f\n" % (
                                nowf_dev,
                                preds[0],
                                preds[1],
                                preds[2],
            ...
            predss = [ eval_acc(x) for x in test_batches_x ]
	    preds = [item[0] for item in predss]
            predps = [item[1] for item in predss]
	    self.result2file(preds, predps, test_batches_y, args.result + '.test')
            nowf_test = self.eval_accuracy(preds, test_batches_y)
            preds = self.eval_macro_recall(preds, test_batches_y)
            pred = (preds[1] + preds[3] + preds[5]) / 3

            say("\ttest accuracy/negativeN/negativeR/neutralN/neturalR/positiveN/positiveR/macroR=%.4f\t%d\t%.4f\t%d\t%.4f\t%d\t%.4f\t%.4f\n" % (
                                nowf_test,
                                preds[0],
                                preds[1],
                                preds[2],
        ...
        for epoch in xrange(args.max_epochs):
			...
			pred = (preds[1] + preds[3] + preds[5]) / 3
				...
				pred,
                        ...
                        if args.test and pred == best_dev:
                            predss = [ eval_acc(x) for x in test_batches_x ]
                            preds = [item[0] for item in predss]
			    nowf_test = self.eval_accuracy(preds, test_batches_y)
                            preds = self.eval_macro_recall(preds, test_batches_y)
